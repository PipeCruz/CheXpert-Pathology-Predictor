...
# Update the checkpoint path for ResNet50
CKPT_PATH = '/home/ezhang3/156b/checkpoints/best-checkpoint.ckpt'
N_CLASSES = 9
...
class MedicalImageClassifier(pl.LightningModule):
    def __init__(self):
        super().__init__()
        self.model = models.resnet50(pretrained=True)  # Use ResNet50
        num_ftrs = self.model.fc.in_features  # Get the number of input features for the classifier
        self.model.fc = nn.Linear(num_ftrs, N_CLASSES)  # Replace the classifier with a new one for our number of classes
